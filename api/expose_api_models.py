...
def expose_models(app, swagger_host="localhost", PORT=5656, API_PREFIX="/api", **kwargs):
    """ Declare API - create SAFRSAPI, exposing each model (note: end point names are table names) """
    app_logger.info(f"")
    app_logger.info(f"Declare   API - api/expose_api_models, endpoint for each table on {swagger_host}:{PORT}")
